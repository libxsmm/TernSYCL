#!/bin/bash
# Validation: SYCL upcvt (GEMV + large-M GEMM + ragged shapes, distinct sets).
#   bash validate.sh
cd "$(dirname "$0")"; S=./build/int2_fp16_upcvt_sycl
...
for mkn in "1024 17408 5120" "1024 5120 14336" "1024 5120 34816" "1000 5120 4096" "77 128 48" "129 256 272" \
  ...
  set -- $mkn
  o=$($S --m $1 --k $2 --n $3 --dtype $dt --iters 3 --sets 2 --distinct-sets 2>&1)
  echo "sycl $dt M=$1 K=$2 N=$3 | $(summ <<<"$o")| $(grep -a 'Validation summary\|error\|failed' <<<"$o" | head -2)"
done; done
