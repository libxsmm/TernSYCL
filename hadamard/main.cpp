...
static void ref_block(const dt16 *x, const int8_t *s, dt16 *y) {
    float v[1024];
    ...
    for (int i = 0; i < 512; ++i) {
        ...
        y[i] = fromf((a + b) * (1.0f / 32.0f));
        y[i + 512] = fromf((a - b) * (1.0f / 32.0f));
    }
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--rows" && i + 1 < argc) rows = std::atoi(argv[++i]);
            ...
            std::cout << "Usage: " << argv[0] << " [--rows R] [--k K] [--dtype fp16|bf16] [--no-signs] [--iters N]\n";
    ...
    for (size_t b = 0; b < (size_t)rows * bpr; ++b) {
        ...
        ref_block(x.data() + off, use_signs ? s.data() + (b % bpr) * 1024 : nullptr, g.data() + off);
    }
    ...
    auto launch = [&] {
        return dt_is_bf16() ? q.parallel_for(r, hadamard::Fwht1024<true>{dx, sp, dy, K})
                            : q.parallel_for(r, hadamard::Fwht1024<false>{dx, sp, dy, K});
    };
    launch().wait();
    double ns = 0.0;
